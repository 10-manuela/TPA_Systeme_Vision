...
class SmartGlaceLiteApp:
    """Application SmartGlaceLite - Miroir interactif multi-visages."""

    def __init__(self):
        # MediaPipe avec parametres optimaux
        self.mp_face_mesh = mp.solutions.face_mesh
        self.face_mesh = self.mp_face_mesh.FaceMesh(
            # --- MODIFICATION : Detection de 5 visages max ---
            max_num_faces=5,
        ...
        self.effects = [
            ...
            {"name": "Cartoon", "icon": "Crt", "description": "Style dessin anime (Rapide)"},
        ...
        # --- MODIFICATION : Suppression des etats d'expression (gere par visage) ---
    ...
    # --- MODIFICATION : Renvoie un dictionnaire au lieu de definir 'self' ---
    def calculate_expression_metrics(self, landmarks, w, h):
        """Calcule les metriques d'expression pour UN visage."""
        if not landmarks:
            return {"smile": False, "open": False, "closed": False}
        ...
        try:
            ...
            smile_detected = smile_ratio > self.smile_threshold
            ...
            mouth_open = mouth_open_ratio > self.mouth_open_threshold
            ...
            pass # Retourne les valeurs par defaut (False)

        return {"smile": smile_detected, "open": mouth_open, "closed": eyes_closed}


    # =========================================================================
    # UTILITAIRES PROFESSIONNELS (Optimises)
    # =========================================================================
    
    def smooth_landmarks(self, landmarks):
        """Lissage temporel des landmarks pour stabilite."""
        # Note : Le lissage multi-visage est complexe.
        # Cette implementation simple lisse le premier visage detecte.
        # Pour un vrai lissage multi-visage, il faudrait un suivi d'identite.
        # Pour l'instant, nous appliquons un lissage simple base sur le "dernier" visage vu.
        ...
        alpha = 0.7
        smoothed = []
        # Limite le lissage si le nombre de points a change (evite les crashs)
        if len(landmarks) != len(self.last_landmarks):
            self.last_landmarks = landmarks
            return landmarks
    ...
    def create_face_mask(self, image, landmarks):
        """Cree un masque precis du visage avec feathering (optimise)."""
        h, w = image.shape[:2]
    ...
    def apply_bilateral_beauty(self, image, mask, strength=0.8):
        """Application RAPIDE du lissage de peau (Median Blur)."""
    ...
    def effect_portrait_mode(self, image, mask):
        """Flou d'arriere-plan simple et rapide."""
        
        background = cv2.GaussianBlur(image, (0, 0), 10) 

        inv_mask = 255 - mask
        inv_mask = cv2.GaussianBlur(inv_mask, (21, 21), 10)
    ...
    def effect_cartoon(self, image):
        """Effet style "Comic Book" rapide (remplace stylization)."""
    ...
    # --- MODIFICATION : Prend 'expression_metrics' en argument ---
    def effect_teeth_whitening(self, image, landmarks, expression_metrics):
        """Effet dynamique : blanchit les dents si l'utilisateur sourit bouche ouverte."""
        result = image.copy()
        
        if not expression_metrics["smile"] or not expression_metrics["open"]:
            return result
    ...
    # --- MODIFICATION : Prend 'expression_metrics' en argument ---
    def effect_sleepy(self, image, expression_metrics):
        """Effet dynamique : flou si les yeux sont fermes."""
        if expression_metrics["closed"]:
    ...
    # =========================================================================
    # TRAITEMENT PRINCIPAL (Mis a jour pour Multi-Visage)
    # =========================================================================
    ...
    def process_frame(self, frame):
        ...
        # --- OPTIMISATION : Traiter l'IA sur une image plus petite ---
        ...
        # --- MODIFICATION : Listes pour stocker les donnees de chaque visage ---
        all_landmarks = []
        all_masks = []
        all_expression_metrics = []
        ...
        if detection.multi_face_landmarks:
            ...
            for landmarks_raw in detection.multi_face_landmarks:
                # Applique le lissage (base sur le dernier visage, simple approximation)
                landmarks = self.smooth_landmarks(landmarks_raw.landmark)
                
                # Calcule les metriques pour ce visage
                expression_metrics = self.calculate_expression_metrics(landmarks, w, h)
                
                # Cree le masque pour ce visage
                mask = self.create_face_mask(frame, landmarks)
                
                all_landmarks.append(landmarks)
                all_masks.append(mask)
                all_expression_metrics.append(expression_metrics)

        # Met a jour la confiance de detection
        if self.face_detected:
            ...
            self.last_landmarks = None # Reinitialiser si aucun visage

        effect_name = self.effects[self.current_effect]["name"]
        
        # --- MODIFICATION : Logique d'application des effets ---
        
        result = frame.copy()
        
        # 1. Appliquer les effets plein ecran d'abord
        if effect_name == "Color Grading":
            result = self.effect_color_grading(result)
        elif effect_name == "Glitch":
            result = self.effect_glitch(result)
        elif effect_name == "Cartoon":
            result = self.effect_cartoon(result)
        elif effect_name == "Vintage Film":
            result = self.effect_vintage_film(result)
        elif effect_name == "Liquid Metal" and not self.face_detected:
            result = self.effect_liquid_metal(result, None, None)
        elif effect_name == "Sleepy" and not self.face_detected:
            # Appliquer "Sleepy" a tout l'ecran si aucun visage n'est detecte
            # (utilise des metriques "False" par defaut)
            result = self.effect_sleepy(result, {"closed": False})
            
        # 2. Cas special : Mode Portrait (base sur tous les masques)
        elif effect_name == "Portrait Mode":
            if self.face_detected:
                # Combine tous les masques de visage en un seul
                combined_mask = np.zeros((h, w), dtype=np.uint8)
                for m in all_masks:
                    combined_mask = cv2.bitwise_or(combined_mask, m)
                result = self.effect_portrait_mode(frame, combined_mask)
            else:
                result = frame.copy() # Pas de visage, pas de flou

        # 3. Appliquer les effets faciaux (boucle sur chaque visage)
        elif self.face_detected:
            # Les effets sont appliques a 'frame' et melanges dans 'result'
            
            for i in range(len(all_landmarks)):
                landmarks = all_landmarks[i]
                mask = all_masks[i]
                expression_metrics = all_expression_metrics[i]
                
                # Calcule l'effet pour ce visage (a partir de l'image originale)
                effect_image = frame.copy()
                
                if effect_name == "Glow Skin":
                    effect_image = self.effect_glow_skin(effect_image, landmarks, mask)
                elif effect_name == "Anime Eyes":
                    effect_image = self.effect_anime_eyes(effect_image, landmarks)
                elif effect_name == "Beauty Filter":
                    effect_image = self.effect_beauty_filter(effect_image, landmarks, mask)
                elif effect_name == "Cyberpunk":
                    effect_image = self.effect_cyberpunk(effect_image, landmarks)
                elif effect_name == "3D Depth":
                    effect_image = self.effect_3d_depth(effect_image, landmarks)
                elif effect_name == "Rainbow Prism":
                    effect_image = self.effect_rainbow_prism(effect_image, landmarks)
                elif effect_name == "Crystal Face":
                    effect_image = self.effect_crystal_face(effect_image, landmarks)
                elif effect_name == "Liquid Metal":
                    effect_image = self.effect_liquid_metal(effect_image, landmarks, mask)
                elif effect_name == "Bulge":
                    effect_image = self.effect_bulge(effect_image, landmarks)
                elif effect_name == "Overlay":
                    effect_image = self.effect_graphic_overlay(effect_image, mask)
                elif effect_name == "Teeth Whiten":
                    effect_image = self.effect_teeth_whitening(effect_image, landmarks, expression_metrics)
                elif effect_name == "Sleepy":
                    effect_image = self.effect_sleepy(effect_image, expression_metrics)
                else:
                    # Si l'effet n'est pas facial (ex: Normal), 'effect_image' reste 'frame'
                    pass 

                # Si l'effet n'est pas "Normal", melange le resultat
                if effect_name != "Normal":
                    mask_3ch = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR) / 255.0
                    result = (effect_image * mask_3ch) + (result * (1 - mask_3ch))
    ...
    # --- MODIFICATION : Suppression de l'affichage des expressions ---
    def draw_modern_ui(self, frame):
        """Interface utilisateur moderne (sans affichage d'expression)."""
        h, w = frame.shape[:2]
        overlay = frame.copy()
        ...
        if self.face_detected:
            ...
            status_text = "VISAGE(S) DETECTE(S)" # Modifie
        else:
            status_color = CLR_JAUNE_FONCE
            status_bg = CLR_BLEU_TRES_FONCE
            status_text = "POSITIONNEZ-VOUS"
        confidence_text = f"Confiance: {int(self.detection_confidence * 100)}%"
        ...
        cv2.putText(overlay, confidence_text, (status_x + 15, status_y + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, CLR_GRIS_CLAIR, 1, cv2.LINE_AA)

        # --- SUPPRESSION de l'affichage d'expression ---
        
        title = "SmartGlaceLite"
...
def main():
    print("\n" + "="*70)
    print("║" + " "*68 + "║")
    print("║" + " SMARTGLACELITE - MIROIR ARTISTIQUE INTELLIGENT ".center(68) + "║")
    print("║" + " "*68 + "║")
    print("="*70)
    print("Initialisation...")

    app = SmartGlaceLiteApp()
    ...
    window_name = "SmartGlaceLite (Bleu Ciel / Jaune)"
    ...
    while True:
        ...
        if key == ord('q') or key == 27:  # Q ou ESC
            ...
            screenshot_idx += 1
            filename = f"smart_glace_lite_capture_{screenshot_idx:03d}.png"
            cv2.imwrite(filename, processed)
            print(f"Capture d'ecran SmartGlaceLite sauvegardee: {filename}")
